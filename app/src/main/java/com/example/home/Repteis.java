...
public class Repteis extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (animaisList != null && !animaisList.isEmpty()) {
            Log.d("Repteis", "Número de animais recebidos: " + animaisList.size());
        } else {
            Log.e("Repteis", "A lista de animais está vazia ou a requisição falhou.");
        }

        // Configuração do botão voltar
        voltar.setOnClickListener(v -> {
            Intent backIntent = new Intent(Repteis.this, Home.class);
        ...
        // Se a lista de animais não estiver vazia, começa a criar os botões para répteis
        if (animaisList != null) {
            int topMargin = 20; // Inicializa a margem superior para o primeiro botão

            // Filtra e cria botões para os répteis (classe 2)
            for (int i = 0; i < animaisList.size(); i++) {
                AnimaisExtintosModel animal = animaisList.get(i);

                // Verifica se o animal é um réptil (classe 2)
                if (animal.getClasse() == 2) {
                    Log.d("Repteis", "Adicionando réptil: " + animal.getNome());

                    // Cria um novo botão programaticamente
                    Button button = new Button(Repteis.this);
                    ...
                    button.setOnClickListener(v -> {
                        Log.d("Repteis", "Botão clicado para o animal: " + animal.getNome());
        ...
        // Configuração do botão de animais (navegação)
    }
}
