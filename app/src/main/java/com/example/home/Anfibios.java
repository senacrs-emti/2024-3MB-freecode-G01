...
public class Anfibios extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (animaisList != null && !animaisList.isEmpty()) {
            Log.d("Anfibios", "Número de animais recebidos: " + animaisList.size());
        } else {
            Log.e("Anfibios", "A lista de animais está vazia ou a requisição falhou.");
        }
        ...
        perfilAnimais.setOnClickListener(v -> {
            Intent intentPerfil = new Intent(Anfibios.this, Cadastro.class);
            startActivity(intentPerfil);
        });

        // Se a lista de animais não estiver vazia, começa a criar os botões para os anfíbios
        if (animaisList != null) {
            int topMargin = 20; // Inicializa a margem superior para o primeiro botão

            // Filtra e cria botões para os anfíbios (classe 4)
