...
public class Plantas extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (animaisList != null && !animaisList.isEmpty()) {
            Log.d("Plantas", "Número de animais recebidos: " + animaisList.size());
        } else {
            Log.e("Plantas", "A lista de animais está vazia ou a requisição falhou.");
        }
        ...
        // Se a lista de animais não estiver vazia, começa a criar os botões para plantas
        if (animaisList != null) {
            int topMargin = 20; // Inicializa a margem superior para o primeiro botão

            // Filtra e cria botões para as plantas (classe 5)
            for (int i = 0; i < animaisList.size(); i++) {
                AnimaisExtintosModel animal = animaisList.get(i);

                // Verifica se o animal é uma planta (classe 5)
                if (animal.getClasse() == 5) {
                    Log.d("Plantas", "Adicionando planta: " + animal.getNome());

                    // Cria um novo botão programaticamente
                    Button button = new Button(Plantas.this);
                    ...
                    button.setOnClickListener(v -> {
                        Log.d("Plantas", "Botão clicado para a planta: " + animal.getNome());
